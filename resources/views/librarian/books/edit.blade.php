    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h1 class="h2">Edit Book: {{ $book->title }}</h1>
            ...
            <div class="card">
                <div class="card-body">
                    {{-- The form submits to the update route, passing the book's ID --}}
                    <form action="{{ route('librarian.books.update', $book->id) }}" method="POST">
                        @csrf
                        @method('PUT') {{-- Important: Tells Laravel to treat this as a PUT request --}}

                        {{-- Book Title --}}
                        <div class="form-group mb-3">
                            <label for="title">Book Title</label>
                            {{-- The `old()` helper populates from prior input, otherwise uses the book's current data --}}
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" class="form-control @error('total_copies') is-invalid @enderror" id="total_copies" name="total_copies" value="{{ old('total_copies', $book->total_copies) }}" required min="1">
                            @error('total_copies')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
